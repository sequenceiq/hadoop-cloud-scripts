...
KEY_NAME=hdp-key-$USER
KEY=$KEY_NAME.pem
INS_TYPE=m1.large
SEC_NAME=hdp-sec-grp-$USER
CIDR=10.0.0.0/16
NUM_OF_SLAVES=2
...
echo [TAG] $SUBNET,$ROUTE_ID,$GW_ID,$VPC_ID with: $TAG_KEY=$USER >> $LOGFILE
aws ec2 create-tags --resources $SUBNET $ROUTE_ID  $GW_ID $VPC_ID --tags Key=$TAG_KEY,Value=$USER  --out text >> $LOGFILE
...
echo [TAG] $SEC_ID  with: $TAG_KEY=$USER >> $LOGFILE
aws ec2 create-tags --resources $SEC_ID  --tags Key=$TAG_KEY,Value=$USER --out text >> $LOGFILE
...
# tag
echo [TAG] $AMBARI_ID  with: $TAG_KEY=$USER >> $LOGFILE
aws ec2 create-tags --resources $AMBARI_ID  --tags Key=$TAG_KEY,Value=$USER --out text >> $LOGFILE
...
echo [TAG] CUST_AMI with $TAG_KEY=$USER >> $LOGFILE
aws ec2 create-tags --resources $CUST_AMI  --tags Key=$TAG_KEY,Value=$USER --out text >> $LOGFILE

SNAPSHOT=$(aws ec2 describe-images --image-ids $CUST_AMI --query Images[].BlockDeviceMappings[0].Ebs.SnapshotId --out text)
echo [TAG] SNAPSHOT:$SNAPSHOT with $TAG_KEY=$USER >> $LOGFILE
aws ec2 create-tags --resources $SNAPSHOT  --tags Key=$TAG_KEY,Value=$USER --out text >> $LOGFILE
...
# start NUM_OF_SLAVES instances
SLAVE_RESERV=$(aws ec2 run-instances --image-id $CUST_AMI --count $NUM_OF_SLAVES --key-name $KEY_NAME --instance-type $INS_TYPE  --security-group-ids $SEC_ID --subnet-id $SUBNET --associate-public-ip-address --query ReservationId --out text)
...
# tag all slave
aws ec2 create-tags --resources $SLAVES  --tags Key=$TAG_KEY,Value=$USER --out text >> $LOGFILE
